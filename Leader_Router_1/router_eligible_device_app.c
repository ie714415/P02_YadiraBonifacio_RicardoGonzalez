...
/*!*************************************************************************************************
\private
\fn     static void Accel_Init(void)
\brief  Initialize the accelerometer from the board.
...
static void Accel_Init(void)
{
	...
	gfxosHandle.i2cHandle = &g_MasterHandle;
	...
	{
		gfxosHandle.xfer.slaveAddress = g_accel_address[i];
...
/*!*************************************************************************************************
\private
\fn     static void APP_CoapAccel8Cb(coapSessionStatus_t sessionStatus, uint8_t *pData,
                                   coapSession_t *pSession, uint32_t dataLen)
\brief  This function is the callback function for CoAP sink message.
...
)
{
	...
	char pMySessionPayloadTemp[25];
	coapSession_t *pMySession = NULL;
	fxos_data_t sensorData;
	int16_t xData, yData, zData;
	char xDataStr[4], yDataStr[4], zDataStr[4];
	...
	if (gCoapGET_c == pSession->code)
	{
		if (gCoapConfirmable_c == pSession->msgType)
		{
			pMySession -> msgType = gCoapMsgTypeAckSuccessChanged_c;
			COAP_Send(pMySession, pMySession->msgType, pMySessionPayload, pMyPayloadSize);
		}
		...
		itoa(xData,xDataStr,10);
		itoa(yData,yDataStr,10);
		itoa(zData,zDataStr,10);

		for(uint8_t indx = 0; 4 > indx; indx++) // Remove \0 from string
		{
			if(xDataStr[indx] == '\0')
			{
				xDataStr[indx] = ' ';
		...
		sprintf(pMySessionPayloadTemp,"X=%c%c%c%c Y=%c%c%c%c Z=%c%c%c%c",xDataStr[0],xDataStr[1],xDataStr[2],xDataStr[3],
				yDataStr[0],yDataStr[1],yDataStr[2],yDataStr[3],
				zDataStr[0],zDataStr[1],zDataStr[2],zDataStr[3]);

		for(uint8_t indx = 0; 25 > indx; indx++)
		{
			pMySessionPayload[indx] = pMySessionPayloadTemp[indx];
		}
